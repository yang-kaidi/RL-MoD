...
dvar int+ desiredAcc[region][t0..tf-1];
...
subject to
{
  forall(t in t0..tf-1)
  {
    ...
    {  
		...
		{
			desiredAcc[i][t] >= (sum(ii in region) acc[ii][t])*act -1 + 1e-3 - 40 * (1-action[i][act][t]);
			desiredAcc[i][t] <= (sum(ii in region) acc[ii][t])*act + 40 * (1-action[i][act][t]);
		}
