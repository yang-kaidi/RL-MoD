...
#                 print(f"lambda, xl {x_left}, xr {x_right}")
    ...
    def policy(self, obs, params, train=True, CPLEXPATH=None, res_path=None):
        """
        ...
        modfile = modPath+'minRebDist.mod'
...
class AMoD:
    ...
    def __init__(self, scenario, beta=0.2): # updated to take scenario and beta (cost for rebalancing) as input
        ...
        self.obs = [self.acc[n][t] for n in self.G.nodes] + [self.demand[i,j][t+1] for i,j in self.demand]

    def matching(self, CPLEXPATH=None):
        t = self.time
    ...
    def step(self, action, isMatching = False): 
        ...
        self.time += 1          
        self.obs = [self.acc[n][t+1] for n in self.G.nodes] + [self.demand[i,j][t+1] for i,j in self.demand]
...
if __name__=='__main__':
    ...
    served = dict.fromkeys(['matching_only', 'MPC'], 0)
    rebcost = dict.fromkeys(['matching_only', 'MPC'], 0)
    revenue = dict.fromkeys(['matching_only', 'MPC'], 0)
    opt_rew = dict.fromkeys(['matching_only', 'MPC'], 0)
    while(not done):
        print(env1.time)   
        rebAction = [0 for i,j in env1.edges]
        obs, reward, done, info = env1.step(rebAction, isMatching = True)
        served['matching_only'] += info['served_demand']
        rebcost['matching_only'] += info['rebalancing_cost']
        revenue['matching_only'] += info['revenue']
        opt_rew['matching_only'] += reward
    ...
    while(not done):
        ...
        served['MPC'] += info['served_demand']
        rebcost['MPC'] += info['rebalancing_cost']
        revenue['MPC'] += info['revenue']
        opt_rew['MPC'] += reward
...
#             reward += (self.servedDemand[i,j][t]*self.price[i,j][t] - self.G.edges[i,j]['time']*self.beta*action[k])
#             self.info['served_demand'] += self.servedDemand[i,j][t]
#             self.info['rebalancing_cost'] += self.G.edges[i,j]['time']*self.beta*action[k]
        
#         self.info['revenue'] = reward
#         # observation: current vehicle distribution - for now, no notion of demand
#         # TODO: define states here
#         self.time += 1          
#         self.obs = (self.acc, self.time)
